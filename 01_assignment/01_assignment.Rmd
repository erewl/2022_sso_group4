...
```{r, results='hide'}
# upperbound
upper = p + (z_alpha)*sqrt((p*q)/n)
# lowerbound
lower = p - (z_alpha)*sqrt((p*q)/n)
CI = c(lower, upper)
```
...
```
The summary table and the boxplot of the temperature data show a greater range of values (see Min and Max values) than in the humidity. Notably, we can see that the humidity values are approximately symmetrically distributed around the median. On the other hand, the values smaller than the median temperature have a larger range than those that are larger than the median.
