...
**H_null**:  The data is normally distributed.

**H_alt**: The data is not normally distributed.

Since the  p-value `r round(shapiro.test(birthweight$birthweight)$p.value, 4)` is greater than 0.05, we will not reject the null hypothesis, stating the data may be normally distributed. 
...
**H_null**: mean_birthweight = 2800

**H_alt**: mean birthweight > 2800
...
```
The p-value (0.0136) of the t-test is less than the significance level 0.05, and therefore we reject the null hypothesis.
We have sufficient evidence to verify the experts claim that the mean birthweight is greater than 2800.
...
p-hat = 140/200 = `r round(140/200,3)`
...
To derive a 99% confidence interval for the p-hat we first need to calculate the chance of failure (q). To estimate the chance of failure (no child benefit), we use the formula q = 1 - p. In this case, the q-hat is `r round(1-p,3)`.\n
...
With the z-value, the probability of success p-hat, the probability of failure q-hat, and the sample size, we can calculate the upper and lower bound of the 99% confidence interval. This results in the following confidence interval [`r round(lower,3)`, `r round(upper,3)`].

## Exercise 1.2.c

Finally, we will test the null hypothesis that the true fraction of working parents who receive childcare benefits is equal to 75%. Namely, H0: $p = p_0 = 0.75$, H1: $p \neq 0.75$.\n
...
tab <- matrix(c(test_99$conf.int, test_90$conf.int, test_85$conf.int, test_99$conf.int[2], test_90$conf.int[2], test_85$conf.int[2]), ncol=2,nrow = 3, byrow=TRUE)
...
```
The summary table and the boxplot of the temperature data show a greater range of values (see Min and Max values) in the temperature than in the humidity. Notably, we can see that the humidity values are approximately symetrically distributed around the median. On the other hand, the values smaller than the median temperature have a larger range than those that are larger than the median.
...
Testing for independence or homogeneity depends on the topic of interest. With independence testing, there is one large sample, and then you are testing whether the row and column variables differ significantly. With testing for homogeneity, samples are taken from different populations, with one sample per column. Here we test whether the columns differ significantly from each other. Since the admirer attempts to imitate Austen's writing style, the contingency table test for homogeneity seems a fitting choice. Generalized, the contingency table test for homogeneity explores whether two datasets are originating from the same population, this maps well to the intention of the admirer. In our case, we want to test for consistency between the different samples, so we want to see whether the columns are the same or differ. We will use the Homogeneity test to look for a difference between the columns.\n
\n
...
The chi-squared test is used to compare observed results and expected results. Its purpose is discovering whether there is a significant difference between the observed and the expected data.\n
To test only the books/chapter of Austen, we created a subset with only the books/chapters that were written by Austen. With this new dataset, we performed a chi-squared test. This gives us a p-value of 0.161. This p-value is > 0.05, meaning we cannot reject the null hypothesis and state that there is a significant difference between the different columns (books/chapters) in the data frame. 
```{r}
