...
The point estimate for mu is `r mean(birthweight$birthweight)`.

The p-value `r shapiro.test(birthweight$birthweight)$p.value` of the Shapiro-Wilk-Test suggests, that the hypothesis that the data is normally distributed is not rejected. 
...
The 90% Confidence interval is [`r low`, `r up`]
...
---
# TODO REMOVE EVERYTHING BELOW HERE ONCE DONE
